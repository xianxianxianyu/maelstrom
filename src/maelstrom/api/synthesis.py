"""Synthesis Engine API endpoints."""
...
from maelstrom.db import session_repo, synthesis_run_repo
...
async def list_runs(session_id: str, limit: int = 5):
    db = await get_db()
    runs = await synthesis_run_repo.list_by_session(db, session_id, limit=limit)
    return [
        {
            "id": r["id"],
            "session_id": r["session_id"],
            "topic": r["topic"],
            "source_gap_id": r.get("source_gap_id"),
            "status": r["status"],
...
async def start_run(body: dict):
    ...
    if not session_id or (not topic and not gap_id):
        raise HTTPException(status_code=400, detail="session_id and (topic or gap_id) required")
    ...
    if not topic:
        topic = f"Gap analysis: {gap_id}"

    run_id = await synthesis_service.start_run(session_id, topic, profile, gap_id=gap_id)
...
async def stream_run(run_id: str):
    ...
    if status["status"] in ("completed", "failed"):
        await synthesis_service.rehydrate_run_state(run_id)
