...
from maelstrom.db import session_repo, planning_run_repo
...
async def list_runs(session_id: str, limit: int = 5):
    db = await get_db()
    runs = await planning_run_repo.list_by_session(db, session_id, limit=limit)
    return [
        {
            "id": r["id"],
...
async def start_run(body: dict):
    ...
    synthesis_id = body.get("synthesis_id")
    ...
    run_id = await planning_service.start_run(session_id, topic, profile, synthesis_id=synthesis_id)
...
async def stream_run(run_id: str):
    ...
    if status["status"] in ("completed", "failed"):
        await planning_service.rehydrate_run_state(run_id)
    ...
    q = planning_service.subscribe(run_id)
    ...
    async def event_generator():
        ...
        try:
            ...
            planning_service.unsubscribe(run_id, q)
